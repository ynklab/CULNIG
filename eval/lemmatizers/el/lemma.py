...
def main():
    ...
    for i, row in tqdm(df.iterrows(), total=len(df)):
        ...
        answer = eval(answer)
        answer_lemmas = [[unicodedata.normalize('NFKC', token.lower()) for token in nlp.analyze(text=ans).lemmata] for ans in answer]

        prediction = row["prediction"]
        pred_lemmatized = [unicodedata.normalize('NFKC', token.lower()) for token in nlp.analyze(text=prediction).lemmata]
